...
fn plot_altitude(plot_ui: &mut PlotUi, trajectory: &[FlightState]) {
    let points: PlotPoints = trajectory.iter()
        .map(|s| [s.time, s.altitude()])
        .collect();
...
fn plot_flight_path(plot_ui: &mut PlotUi, trajectory: &[FlightState]) {
    ...
    if let Some(apogee_state) = trajectory.iter()
        .max_by(|a, b| a.altitude().partial_cmp(&b.altitude()).unwrap())
    {
        plot_ui.points(egui_plot::Points::new(
            vec![[apogee_state.downrange_distance(), apogee_state.altitude()]]
        ).color(egui::Color32::RED).radius(5.0));
    }
}