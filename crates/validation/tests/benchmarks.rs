...
mod benchmarks {
    ...
    fn benchmark_simulation_speed() {
        ...
        for _ in 0..NUM_RUNS {
            ...
            let result =
                engine.simulate(initial.clone(), None, &tree, &atmosphere, &gravity, &wind);
        ...
        assert!(
            avg.as_secs_f64() < 10.0,
            "Simulation must run faster than real-time: {:?}",
            avg
        );
    }
    ...
    fn benchmark_aero_calculation() {
        ...
        assert!(
            avg_us < 100.0,
            "Aero calculation too slow: {:.1} µs",
            avg_us
        );
    }
